...
The **select** method uses the default equality check (`===`) to check whether a selector's value has changed. If `previousValue === currentValue` the selector will not emit for `currentValue`.
...
We can use the form of **select** described in the previous section to combine the selectors but we can simplify this by calling **select** with a record of selectors (or any observable).
